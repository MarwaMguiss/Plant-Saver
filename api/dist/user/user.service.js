...
let UserService = class UserService {
    ...
    async login(loginDto) {
        ...
        console.log(user);
        if (!user) {
            throw new common_1.UnauthorizedException('Bad credentials - user not found');
        }
        else {
            if (await this.verifyPassword(loginDto.password, user.password)) {
                ...
                throw new common_1.UnauthorizedException('Bad credentials - pass mismatch');
            }
        }
    }
    ...
    async register(createUser) {
        ...
        if (checkForUser) {
            throw new common_1.BadRequestException('Email is already chosen, please choose a new one');
        }
        else {
